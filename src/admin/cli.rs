...
impl CLI {
	...
	pub async fn channel_new(
		&self, node_id: Vec<u8>, value_sat: u64, push_msat: u64, is_public: bool,
	...
	pub async fn channel_close(
		&self, channel_id: Vec<u8>, is_force: bool,
	) -> Result<(), Box<dyn std::error::Error>> {
		let mut client = self.connect().await?;
	...
	pub async fn peer_connect(
		&self, node_id: Vec<u8>, address: String,
	) -> Result<(), Box<dyn std::error::Error>> {
		let mut client = self.connect().await?;
	...
	pub async fn payment_keysend(
		&self, node_id: Vec<u8>, value_msat: u64,
	) -> Result<(), Box<dyn std::error::Error>> {
		let mut client = self.connect().await?;
