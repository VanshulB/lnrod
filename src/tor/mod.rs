...
impl TorManager {
	...
	pub async fn start(data_dir_prefix: &Path) -> Self {
		...
		let child = run_tor(
			...
			&mut [
				...
				control_port_path.to_str().unwrap(),
				"--DataDirectory",
		...
		let tor_control_addr: SocketAddr =
		...
		let socks_addr_string =
			control_connection.get_info("net/listeners/socks").await.expect("socks port");
	...
	pub async fn init_service(&self, port: u16) -> OnionAddressV3 {
		...
		self.control_connection
			...
			.add_onion_v3(
				...
				false,
				None,
				&mut [(port, SocketAddr::new(IpAddr::from(Ipv4Addr::new(127, 0, 0, 1)), port))]
					.iter(),
			)
			.await
			.unwrap();
	...
	pub fn get_connector(&self) -> TorConnector {
		TorConnector { tor_proxy_addr: self.tor_proxy_addr }
	}
}
...
fn decode_socks_port(socks_addr_string: String) -> SocketAddr {
	...
	let socks_port_str = socks_port_pattern
		.captures(&socks_addr_string)
		.expect("parse")
		.get(1)
		.expect("capture")
		.as_str();
	let socks_port = socks_port_str.parse().expect("integer port");
	let tor_proxy_addr: SocketAddr =
...
async fn connect_tor_socks_proxy<'a>(
	proxy: SocketAddr, dest: impl IntoTargetAddr<'a>,
) -> Result<TcpStream> {
