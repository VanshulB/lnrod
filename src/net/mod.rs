...
impl Connection {
	fn event_trigger(us: &mut MutexGuard<Self>) {
		let res = us.event_notify.try_send(());
		match res {
			Ok(_) => {}
	...
	async fn schedule_read<CMH, RMH, L>(
		peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor, Arc<CMH>, Arc<RMH>, Arc<L>>>,
		us: Arc<Mutex<Self>>, mut reader: io::ReadHalf<TcpStream>,
	...
	) -> (io::ReadHalf<TcpStream>, mpsc::Receiver<()>, mpsc::Receiver<()>, Arc<Mutex<Self>>) {
...
///
/// See the module-level documentation for how to handle the event_notify mpsc::Sender.
pub fn setup_outbound<CMH, RMH, L>(
	peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor, Arc<CMH>, Arc<RMH>, Arc<L>>>,
	...
	L: Logger + 'static + ?Sized + Send + Sync,
{
	...
	let handle_opt = if let Ok(initial_send) =
		peer_manager.new_outbound_connection(their_node_id, SocketDescriptor::new(us.clone()))
	{
...
///
/// See the module-level documentation for how to handle the event_notify mpsc::Sender.
pub async fn connect_outbound<CMH, RMH, L>(
	peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor, Arc<CMH>, Arc<RMH>, Arc<L>>>,
...
impl Clone for SocketDescriptor {
	fn clone(&self) -> Self {
		Self { conn: Arc::clone(&self.conn), id: self.id }
	}
}
...
mod tests {
	...
	impl ChannelMessageHandler for MsgHandler {
		fn handle_open_channel(
			&self, _their_node_id: &PublicKey, _their_features: InitFeatures, _msg: &OpenChannel,
		) {
		}
		fn handle_accept_channel(
			&self, _their_node_id: &PublicKey, _their_features: InitFeatures, _msg: &AcceptChannel,
		) {
		}
		...
		fn handle_shutdown(
			&self, _their_node_id: &PublicKey, _their_features: &InitFeatures, _msg: &Shutdown,
		) {
		}
	...
	async fn do_basic_connection_test() {
		...
		// 127.0.0.1.
		let (conn_a, conn_b) =
			if let Ok(listener) = tokio::net::TcpListener::bind("127.0.0.1:9735").await {
				(
					tokio::net::TcpStream::connect("127.0.0.1:9735").await.unwrap(),
					listener.accept().await.unwrap().0,
				)
			} else if let Ok(listener) = tokio::net::TcpListener::bind("127.0.0.1:9999").await {
				(
					tokio::net::TcpStream::connect("127.0.0.1:9999").await.unwrap(),
					listener.accept().await.unwrap().0,
				)
			} else if let Ok(listener) = tokio::net::TcpListener::bind("127.0.0.1:46926").await {
				(
					tokio::net::TcpStream::connect("127.0.0.1:46926").await.unwrap(),
					listener.accept().await.unwrap().0,
				)
			} else {
