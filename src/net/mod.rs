...
mod tests {
    ...
    async fn do_basic_connection_test() {
        ...
        // 127.0.0.1.
        let (conn_a, conn_b) = if let Ok(listener) = tokio::net::TcpListener::bind("127.0.0.1:9735").await {
            (tokio::net::TcpStream::connect("127.0.0.1:9735").await.unwrap(), listener.accept().await.unwrap().0)
        } else if let Ok(listener) = tokio::net::TcpListener::bind("127.0.0.1:9999").await {
            (tokio::net::TcpStream::connect("127.0.0.1:9999").await.unwrap(), listener.accept().await.unwrap().0)
        } else if let Ok(listener) = tokio::net::TcpListener::bind("127.0.0.1:46926").await {
            (tokio::net::TcpStream::connect("127.0.0.1:46926").await.unwrap(), listener.accept().await.unwrap().0)
